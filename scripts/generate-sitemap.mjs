...
function buildImagesSitemap(entries, siteUrl) {
  const withImages = entries.filter((e) => e.images?.length);
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    "<!-- Auto-generated by scripts/generate-sitemap.mjs — do not edit by hand. -->",
    "<!-- Source: src/content/site-urls.ts (entries with images[]) -->",
    '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"',
    '        xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">',
    "",
  ];
  for (const e of withImages) {
    lines.push("  <url>");
    lines.push(`    <loc>${escapeXml(siteUrl + e.path)}</loc>`);
...
export async function generateSitemap() {
  ...
  const imagesXml = buildImagesSitemap(entries, SITE_URL);

  const outDir = resolve(ROOT, "public");
  mkdirSync(outDir, { recursive: true });
  writeFileSync(resolve(outDir, "sitemap.xml"), xml, "utf8");
  writeFileSync(resolve(outDir, "sitemap-index.xml"), indexXml, "utf8");
  writeFileSync(resolve(outDir, "sitemap-images.xml"), imagesXml, "utf8");

  const imageCount = entries
    .filter((e) => e.images?.length)
    .reduce((n, e) => n + e.images.length, 0);
  return { count: entries.length, imageCount };
}
